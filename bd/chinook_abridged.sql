...
(61, 'Omar', 'Sharif', '$2y$10$WtD6WywiBP7qNi8yZj7gYuIhjTy1xsAwAKSEgXj/ftRZWTLjz1cpu', 'Home', 'Ruten 2tv', 'Copenhagen', 'Dhaka', 'Denmark', '2700', '71800374', '71800374', 'rony@yahoo.com');
...
(414, 2, '2021-02-11 00:00:00', 'Ruten 2tv', 'Copenhagen', 'Copenhagen', 'Denmark', '2700', '2.97');
...
--
-- Stand-in structure for view `invoice_all`
-- (See below for the actual view)
--
CREATE TABLE `invoice_all` (
`AlbumId` int
...
(3505, 'rony new song updated', 349, 5, 1, 'Omar Sharif', 99999, 6666666, '1.11');

-- --------------------------------------------------------

--
-- Stand-in structure for view `track_all`
-- (See below for the actual view)
--
CREATE TABLE `track_all` (
`AlbumId` int
,`AlbumTitle` varchar(160)
,`ArtistId` int
,`ArtistName` varchar(120)
...
--
-- Structure for view `invoice_all`
--
DROP TABLE IF EXISTS `invoice_all`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `invoice_all`  AS SELECT `il`.`InvoiceLineId` AS `id`, `iv`.`InvoiceId` AS `InvoiceId`, `ta`.`TrackId` AS `TrackId`, `ta`.`Name` AS `TrackName`, `ta`.`AlbumId` AS `AlbumId`, `ta`.`AlbumTitle` AS `AlbumTitle`, `ta`.`ArtistId` AS `ArtistId`, `ta`.`ArtistName` AS `ArtistName`, `ta`.`MediaTypeId` AS `MediaTypeId`, `ta`.`MediaTypeName` AS `MediaTypeName`, `ta`.`GenreId` AS `GenreId`, `ta`.`GenreName` AS `GenreName`, `ta`.`Composer` AS `Composer`, `ta`.`Milliseconds` AS `Milliseconds`, `ta`.`Bytes` AS `Bytes`, `ta`.`UnitPrice` AS `UnitPrice` FROM ((`invoice` `iv` join `invoiceline` `il` on((`iv`.`InvoiceId` = `il`.`InvoiceId`))) join `track_all` `ta` on((`il`.`TrackId` = `ta`.`TrackId`))) ORDER BY `iv`.`InvoiceId` ASC ;

-- --------------------------------------------------------

--
-- Structure for view `track_all`
--
DROP TABLE IF EXISTS `track_all`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `track_all`  AS SELECT `tr`.`TrackId` AS `TrackId`, `tr`.`Name` AS `Name`, `tr`.`AlbumId` AS `AlbumId`, `al`.`Title` AS `AlbumTitle`, `ar`.`ArtistId` AS `ArtistId`, `ar`.`Name` AS `ArtistName`, `tr`.`MediaTypeId` AS `MediaTypeId`, `mt`.`Name` AS `MediaTypeName`, `tr`.`GenreId` AS `GenreId`, `ge`.`Name` AS `GenreName`, `tr`.`Composer` AS `Composer`, `tr`.`Milliseconds` AS `Milliseconds`, `tr`.`Bytes` AS `Bytes`, `tr`.`UnitPrice` AS `UnitPrice` FROM ((((`track` `tr` join `album` `al` on((`tr`.`AlbumId` = `al`.`AlbumId`))) join `artist` `ar` on((`al`.`ArtistId` = `ar`.`ArtistId`))) join `genre` `ge` on((`tr`.`GenreId` = `ge`.`GenreId`))) join `mediatype` `mt` on((`tr`.`MediaTypeId` = `mt`.`MediaTypeId`))) ORDER BY `tr`.`Name` ASC ;
...
--
ALTER TABLE `admin`
  ADD UNIQUE KEY `IFK_AdminPassword` (`Password`);
...
--
ALTER TABLE `album`
  MODIFY `AlbumId` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=350;

--
-- AUTO_INCREMENT for table `artist`
--
ALTER TABLE `artist`
  MODIFY `ArtistId` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=300;

--
-- AUTO_INCREMENT for table `customer`
--
ALTER TABLE `customer`
  MODIFY `CustomerId` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=65;

--
-- AUTO_INCREMENT for table `genre`
--
ALTER TABLE `genre`
  MODIFY `GenreId` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=26;

--
-- AUTO_INCREMENT for table `invoice`
--
ALTER TABLE `invoice`
  MODIFY `InvoiceId` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=415;

--
-- AUTO_INCREMENT for table `invoiceline`
--
ALTER TABLE `invoiceline`
  MODIFY `InvoiceLineId` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2244;

--
-- AUTO_INCREMENT for table `mediatype`
--
ALTER TABLE `mediatype`
  MODIFY `MediaTypeId` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT for table `track`
--
ALTER TABLE `track`
  MODIFY `TrackId` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3507;
